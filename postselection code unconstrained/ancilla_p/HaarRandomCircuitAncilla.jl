...
function entangle_ancilla!(ψ::Vector{ComplexF64}, i::Integer)
    ...
    for nL = 0:(dimL - 1), nR = 0:(dimR - 1)
        ind1 = 1 + nR + nL * 2^i                      #00
        ind2 = ind1 + 2^(i - 1)                       #10
        ind3 = ind1 + chaindim                        #01
        ind4 = ind1 + 2^(i - 1) + chaindim            #11
        ψ[ind2], ψ[ind3], ψ[ind4] = (ψ[ind3], ψ[ind4], ψ[ind2])
    end
...
function full_evolution_ancilla!(ψ::Vector{ComplexF64}, T::Integer, p::Float64, N::Integer; start_layer_even::Bool = true)
...
function ancilla_entropy(ψ::Vector{ComplexF64})
    ...
    rho = zeros(Float64,2,2)
    rho[1,1] = abs(dot(ψ[1:2^L],ψ[1:2^L]))
    rho[1,2] = abs(dot(ψ[1:2^L],ψ[2^L+1:end]))
    rho[2,1] = rho[1,2]
    rho[2,2] = abs(dot(ψ[2^L+1:end],ψ[2^L+1:end]))
...
function Rényi_entropy(n::Number,ps)
    S = 0
    if n==0
...
function mutual_information(ψ::Vector{ComplexF64},
    A::Vector{Int64},B::Vector{Int64},L::Integer,ns::Vector{Int64})
    ...
    for i = eachindex(ns)
        n = ns[i]
        SA[i] = Rényi_entropy(n,pA)
        SB[i] = Rényi_entropy(n,pB)
        SAB[i] = Rényi_entropy(n,pAB)
    end
