...
def free_energy_routine(N, t, tensors):
    """record the free-energy (log-norm) after reaching the steady state."""
    obj = np.zeros((3,) * N)
    obj[(0,)*N] = 1
    nq1 = range(N)
    oq1 = nq1[::2]
    ...
    for t1 in range(0, t, 2):
        for l1 in oq1:
            ...
            if t1 >= 4*N:
                free_energy -= np.log(np.linalg.norm(obj))
    ...
    return free_energy
def parse_arguments():
    parser = argparse.ArgumentParser(description="Run free energy Calculation")
...
if __name__ == "__main__":
    ...
    t = 8*N  # circuit depth
    n_realizations = 10000  # number of realizations

    free_energy_record = np.zeros(n_realizations)
    ...
    with open(output_file, 'w') as f:
        for iter in range(n_realizations):
            ...
            free_energy_record[iter] = free_energy_routine(
                N, t, tensors_partial)
            print(iter, free_energy_record[iter])
            f.write(f"{free_energy_record[iter]}\n")
            f.flush()
